...
class Application:
    ...
    def get_suggestions(self):
        spell = SpellChecker()
        word = self.word.strip()  # Remove any leading/trailing spaces
        if not word:
            return []  # Return empty if the word is empty
        # Ensure that spell.distance is still a method
        if not callable(spell.distance):
            raise TypeError("spell.distance is not callable, check for conflicts")
        suggestions = list(spell.candidates(word))  # Get possible suggestions
        # Ensure spell.distance is used correctly
...
# vedio-loop
    ...
    def predict(self, test_image):
        test_image = cv2.resize(test_image, (128, 128)).reshape(1, 128, 128, 1)
